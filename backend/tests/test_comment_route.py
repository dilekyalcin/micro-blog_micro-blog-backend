...
def test_add_comment(client, dummy_user, logged_in_client, dummy_post):
    ...
    response = client.post('/comment/add_comment', headers=headers, json=data)
    assert response.status_code == 201

    response = client.post('/comment/add_comment', headers=headers,json={
...
def test_delete_comment(client, dummy_user, logged_in_client, dummy_comment):
    headers = logged_in_client
    response = client.delete(f'/comment/delete_comment/{str(dummy_comment.id)}', headers=headers)
    assert response.status_code == 200
    
    response = client.delete(f'/comment/delete_comment/{str(str(ObjectId()))}', headers=headers)
    assert response.status_code == 404
...
def test_update_comment(client, dummy_user, logged_in_client, dummy_comment):
    ...
    response = client.put(f'/comment/update_comment/{dummy_comment.id}', json=update_data, headers=logged_in_client)
    assert response.status_code == 200
...
def test_get_comments_by_post(client, dummy_user, logged_in_client, dummy_post, dummy_comment):
    response = client.get(f'/comment/get_all_comments/{dummy_post.id}', headers=logged_in_client)
    assert response.status_code == 200
