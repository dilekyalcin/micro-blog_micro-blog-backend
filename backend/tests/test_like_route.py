def test_add_like(client, dummy_user, logged_in_client, dummy_post):
    response = client.post('/like/add_like', json={"post_id": str(dummy_post.id)}, headers=logged_in_client)
...
def test_remove_like(client, dummy_user, logged_in_client, dummy_post, dummy_like):
    response = client.delete('/like/remove_like', json={"post_id": str(dummy_post.id)}, headers=logged_in_client)
...
def test_get_likes_by_post(client, dummy_user, logged_in_client, dummy_post, dummy_like):
    response = client.get(f'/like/get_all_likes/{str(dummy_post.id)}', headers=logged_in_client)
    assert response.status_code == 200
